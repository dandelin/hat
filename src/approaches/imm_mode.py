...
class Appr(object):
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid):
        ...
        # Model update
        if t == 0:
            self.fisher = utils.fisher_matrix_diag(
                t, xtrain, ytrain, self.model, self.criterion
            )
        else:
            fisher_new = utils.fisher_matrix_diag(
                t, xtrain, ytrain, self.model, self.criterion
            )
            for (n, p), (_, p_old) in zip(
                self.model.named_parameters(), self.model_old.named_parameters()
            ):
