...
class Appr(object):
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid):
        ...
        for i in range(len(bmask)):
            bmask[i] = torch.autograd.Variable(
                bmask[i].data.clone(), requires_grad=False
            )
            self.logs["mask"][t][i] = {}
            self.logs["mask"][t][i][-1] = deepcopy(
                bmask[i].data.cpu().numpy().astype(np.float32)
            )
            if t == 0:
                self.logs["mask_pre"][t][i] = deepcopy(
                    (0 * bmask[i]).data.cpu().numpy().astype(np.float32)
                )
            else:
                self.logs["mask_pre"][t][i] = deepcopy(
                    self.mask_pre[i].data.cpu().numpy().astype(np.float32)
                )
        ...
        if not self.single_task or (self.single_task and t == 0):
            ...
            try:
                for e in range(self.nepochs):
                    ...
                    clock1 = time.time()
                    train_loss, train_acc, train_reg = self.eval_withreg(
                        t, xtrain, ytrain
                    )
                    clock2 = time.time()
                    print(
                    ...
                    # Valid
                    valid_loss, valid_acc, valid_reg = self.eval_withreg(
                        t, xvalid, yvalid
                    )
                    print(
    ...
    def train_epoch(self, t, x, y, thres_cosh=50, thres_emb=6):
        ...
        for i in range(0, len(r), self.sbatch):
            ...
            for n, p in self.model.named_parameters():
                if n.startswith("e"):
                    num = (
                        torch.cosh(torch.clamp(s * p.data, -thres_cosh, thres_cosh)) + 1
                    )
