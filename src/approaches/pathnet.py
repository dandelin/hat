...
class Appr(object):
    ...
    def __init__(
        ...
        self.N = (
            self.model.N
        )  # from paper, number of distinct modules permitted in a pathway
        ...
        self.generations = (
            generations
        )  # Grid search = [5,10,20,50,100,200]; best was 20
        self.P = (
            2
        )  # from paper Secs 2.4 and 2.5, numbers of the individuals in each generation/paths to be trained
        ...
        self.nepochs = (
            nepochs // self.generations
        )  # To maintain same number of training updates
        self.sbatch = sbatch
        self.lr = lr
        self.lr_min = lr_min
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid):
        ...
        if t > 0:  # reinit modules not in bestpath with random, according to the paper
            ...
            for (n, p), (m, q) in zip(
                self.model.named_parameters(), self.initial_model.named_parameters()
            ):
                if n == m:
        ...
        try:
            for g in range(self.generations):
                ...
                print(
                    "| Winning path: {:3d} | Best loss: {:.3f} |".format(
                        ...
                        for j in range(self.L):
                            for k in range(self.N):
                                ...
                                if np.random.rand() < probability:
                                    Path[p, j, k] = (
                                        Path[p, j, k] + np.random.randint(-2, 3)
