...
parser.add_argument(
    "--nruns", default=10, type=int, required=False, help="(default=%(default)d)"
)
parser.add_argument(
    "--nproc", default=3, type=int, required=False, help="(default=%(default)d)"
)
parser.add_argument(
    "--ngpus", default=3, type=int, required=False, help="(default=%(default)d)"
)
parser.add_argument(
    "--sleep", default=0.9, type=float, required=False, help="(default=%(default)f)"
)
...
def mymap(calls, gpuproc):
    ...
    for call in calls:
        ...
        p[i_k] = Process(
            ...
            args=(fn, "CUDA_VISIBLE_DEVICES=" + str(i_gpu) + " " + call),
        )
        p[i_k].start()
    # Wait for last
    for i, j, k in gpuproc:
