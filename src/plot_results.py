...
parser.add_argument(
    "--seeds", default="0-9", type=str, required=False, help="(default=%(default)s)"
)
parser.add_argument(
    "--output", default="", type=str, required=False, help="(default=%(default)s)"
)
...
for a in aref:
    if a not in args.approaches:
        print(
            "ERROR: Need",
            aref,
            "approaches for normalizing the accuracies across data sets",
        )
        sys.exit()
...
# """
markers = ["o", "v", "^", "<", ">", "s", "*", "d", "x", "+", "h"]
while len(markers) < len(args.approaches):
    markers += markers
"""
...
for f in args.folders:
    ...
    if ntasks == 0:
        ntasks = data[f]["random"].shape[1]
    # Normalize
    for s in range(len(args.seeds)):
        for a in args.approaches:
            if a in aref:
                continue
            ref_random = np.repeat(
                np.reshape(
                    np.diag(data[f]["random"][s]), (1, data[f]["random"][s].shape[1])
                ),
                data[f]["random"][s].shape[0],
                axis=0,
            )
...
for f in data.keys():
    for a in data[f].keys():
        ...
        if use_conf_interv:
            ci = (
                stats.t._ppf((1 + 0.95) / 2, acc.shape[0] - 1)
                * ci
                / np.sqrt(acc.shape[0])
            )
