...
class Net(torch.nn.Module):
    def __init__(self, inputsize, taskcla):
        ...
        for t, n in self.taskcla:
            ...
            s = s // 2
            self.conv2.append(
                torch.nn.Conv2d(self.sizec1, self.sizec2, kernel_size=size // 10)
            )
            ...
            if t > 0:
                ...
                self.V2x1.append(
                    torch.nn.Conv2d(
                        t * self.sizec1, self.sizec1, kernel_size=1, stride=1
                    )
                )
                self.U2.append(
                    torch.nn.Conv2d(self.sizec1, self.sizec2, kernel_size=size // 10)
                ...
                self.V3x1.append(
                    torch.nn.Conv2d(
                        t * self.sizec2, self.sizec2, kernel_size=1, stride=1
                    )
                )
                ...
                self.Vf1.append(
                    torch.nn.Linear(t * self.sizec3 * s * s, self.sizec3 * s * s)
                )
    ...
    def forward(self, x, t):
        ...
        if t > 0:  # compute activations for previous columns/tasks & sum laterals
            h_prev2 = [
                self.maxpool(self.drop1(self.relu(self.conv2[j](h_prev1[j]))))
                for j in range(t)
            ]
            h_pre = h_pre + self.U2[t - 1](
                self.relu(
                    self.V2x1[t - 1](
                        torch.cat(
                            [
                                self.V2scale[t - 1].weight[0][j] * h_prev1[j]
                                for j in range(t)
                            ],
                            1,
                        )
        ...
        if t > 0:  # compute activations for previous columns/tasks & sum laterals
            h_prev3 = [
                self.maxpool(self.drop2(self.relu(self.conv3[j](h_prev2[j])))).view(
                    x.size(0), -1
                )
                for j in range(t)
            ]
            h_pre = h_pre + self.U3[t - 1](
                self.relu(
                    self.V3x1[t - 1](
                        torch.cat(
                            [
                                self.V3scale[t - 1].weight[0][j] * h_prev2[j]
                                for j in range(t)
                            ],
                            1,
                        )
        ...
        if t > 0:  # compute activations for previous columns/tasks & sum laterals
            hf_prev1 = [
                self.drop2(self.relu(self.fc1[j](h_prev3[j]))) for j in range(t)
            ]
            h_pre = h_pre + self.Uf1[t - 1](
                self.relu(
                    self.Vf1[t - 1](
                        torch.cat(
                            [
                                self.Vf1scale[t - 1].weight[0][j] * h_prev3[j]
                                for j in range(t)
                            ],
                            1,
                        )
        ...
        if t > 0:  # compute activations for previous columns/tasks & sum laterals
            hf_prev2 = [
                self.drop2(self.relu(self.fc2[j](hf_prev1[j]))) for j in range(t)
            ]
            h_pre = h_pre + self.Uf2[t - 1](
                self.relu(
                    self.Vf2[t - 1](
                        torch.cat(
                            [
                                self.Vf2scale[t - 1].weight[0][j] * hf_prev1[j]
                                for j in range(t)
                            ],
                            1,
                        )
        ...
        for tid, i in self.taskcla:
            if t > 0 and tid < t:
                ...
                if tid > 0:
                    ...
                    h_pre = h_pre + self.Ufl[tid - 1](
                        self.Vfl[tid - 1](
                            torch.cat(
                                [
                                    self.Vflscale[tid - 1].weight[0][j] * hf_prev2[j]
                                    for j in range(tid)
                                ],
                                1,
                            )
